...
Deployment note: server endpoints MUST be deployed to `engram.condetuti.com` BEFORE any client sets `NV_CLOUD_AUTOSYNC=1`. If not, transport returns 404 → manager enters `PhaseBackoff` with `reason_code=server_unsupported` per AD-8 of the proposal.
