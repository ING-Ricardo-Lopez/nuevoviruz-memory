...
| Auth/session model | Reintroduce username/password JWT flow; keep current bearer-token session bridge | Keep current `NV_CLOUD_TOKEN` + signed `engram_dashboard_token`; adapt dashboard middleware to inject a synthetic principal | Proposal explicitly rejects a new identity model. This preserves current security/runtime rules and keeps `/dashboard/login` as an HTTP form fallback. |
