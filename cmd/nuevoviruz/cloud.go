...
import (
	...
	"github.com/ING-Ricardo-Lopez/nuevoviruz-memory/internal/cloud/auth"
	"github.com/ING-Ricardo-Lopez/nuevoviruz-memory/internal/cloud/cloudserver"
	...
	"github.com/ING-Ricardo-Lopez/nuevoviruz-memory/internal/cloud/dashboard"
...
func (p cloudDashboardStatusProvider) Status() dashboard.SyncStatus {
	...
	for _, project := range p.projects {
		...
		if err != nil {
			log.Printf("[nuevoviruz] cloud dashboard status manifest read failed for project %q: %v", project, err)
			return dashboard.SyncStatus{
				Phase:         "degraded",
...
var newCloudRuntime = func(cfg cloud.Config) (cloudServerRuntime, error) {
	...
	token := strings.TrimSpace(os.Getenv("NV_CLOUD_TOKEN"))
...
func cmdCloudStatus(cfg store.Config) {
	...
	insecureNoAuth := envBool("NV_CLOUD_INSECURE_NO_AUTH")
	fmt.Printf("Cloud status: configured (target=%s)\n", constants.TargetKeyCloud)
	fmt.Printf("Server: %s\n", cc.ServerURL)
	if token == "" {
		if insecureNoAuth {
			fmt.Println("Auth status: ready (insecure local-dev mode: NV_CLOUD_INSECURE_NO_AUTH=1)")
		...
		fmt.Println("Hint: if the remote server enforces bearer auth, set NV_CLOUD_TOKEN")
		return
	}
...
func validateCloudServeAuthConfig() error {
	token := strings.TrimSpace(os.Getenv("NV_CLOUD_TOKEN"))
	adminToken := strings.TrimSpace(os.Getenv("NV_CLOUD_ADMIN"))
	insecureNoAuth := envBool("NV_CLOUD_INSECURE_NO_AUTH")
	allowlist := normalizeAllowedProjects(cloud.ConfigFromEnv().AllowedProjects)
	jwtSecretEnv := strings.TrimSpace(os.Getenv("NV_JWT_SECRET"))
	if insecureNoAuth && token != "" {
		return fmt.Errorf("conflicting cloud auth configuration: NV_CLOUD_INSECURE_NO_AUTH=1 cannot be used together with NV_CLOUD_TOKEN")
	}
	if token != "" && len(allowlist) > 0 {
		if jwtSecretEnv == "" {
			return fmt.Errorf("authenticated cloud serve requires explicit NV_JWT_SECRET (non-default); refusing implicit default secret")
		}
		if cloud.IsDefaultJWTSecret(jwtSecretEnv) {
			return fmt.Errorf("authenticated cloud serve requires a non-default NV_JWT_SECRET; refusing development default")
		}
		return nil
	}
	if insecureNoAuth {
		if len(allowlist) == 0 {
			return fmt.Errorf("cloud project allowlist is required even in insecure mode: set NV_CLOUD_ALLOWED_PROJECTS to one or more project names")
		}
		if adminToken != "" {
			return fmt.Errorf("NV_CLOUD_ADMIN is not supported when NV_CLOUD_INSECURE_NO_AUTH=1; remove NV_CLOUD_ADMIN or enable authenticated mode")
		}
		fmt.Fprintln(os.Stderr, "warning: NV_CLOUD_INSECURE_NO_AUTH=1 disables cloud API authentication; do not use in production")
		return nil
	}
	if token == "" {
		return fmt.Errorf("cloud auth token is required: set NV_CLOUD_TOKEN (or NV_CLOUD_INSECURE_NO_AUTH=1 for local insecure development)")
	}
	return fmt.Errorf("cloud project allowlist is required: set NV_CLOUD_ALLOWED_PROJECTS to one or more project names")
}
