...
import (
	...
	engramsrv "github.com/ING-Ricardo-Lopez/nuevoviruz-memory/internal/server"
...
func TestCmdServeAutosyncLifecycleGating(t *testing.T) {
	...
	t.Run("cloud autosync env with token and server starts successfully", func(t *testing.T) {
		...
		cfg := testConfig(t)
		t.Setenv("NV_CLOUD_AUTOSYNC", "1")
		t.Setenv("NV_CLOUD_TOKEN", "test-token")
		t.Setenv("NV_CLOUD_SERVER", "http://127.0.0.1:9999")
...
func TestAutosyncEnvAbsent(t *testing.T) {
	// REQ-210: NV_CLOUD_AUTOSYNC not set → autosync does not start.
	stubRuntimeHooks(t)
	stubExitWithPanic(t)
	cfg := testConfig(t)
	t.Setenv("NV_CLOUD_AUTOSYNC", "")
	t.Setenv("NV_CLOUD_TOKEN", "tok")
	t.Setenv("NV_CLOUD_SERVER", "http://localhost:9999")
...
func TestAutosyncEnvNotOne(t *testing.T) {
	// REQ-210: NV_CLOUD_AUTOSYNC=true (not "1") → autosync does not start.
	stubRuntimeHooks(t)
	stubExitWithPanic(t)
	cfg := testConfig(t)
	t.Setenv("NV_CLOUD_AUTOSYNC", "true")
	t.Setenv("NV_CLOUD_TOKEN", "tok")
	t.Setenv("NV_CLOUD_SERVER", "http://localhost:9999")
	...
	_, _, recovered := captureOutputAndRecover(t, func() { cmdServe(cfg) })
	if _, ok := recovered.(exitCode); ok {
		t.Fatal("serve should not fatal when NV_CLOUD_AUTOSYNC=true (not '1')")
	}
}
...
func TestCloudCommandIsolationDoesNotMutateLocalState(t *testing.T) {
	...
	cfg := testConfig(t)
	cfg.DataDir = filepath.Join(tmpHome, ".nuevoviruz")
	...
	if _, err := os.Stat(filepath.Join(cfg.DataDir, "nuevoviruz.db")); err == nil {
...
func TestMainCloudHelpDoesNotCreateLocalDatabase(t *testing.T) {
	stubRuntimeHooks(t)
	dataDir := filepath.Join(t.TempDir(), ".nuevoviruz")
	t.Setenv("NV_DATA_DIR", dataDir)
	...
	if _, err := os.Stat(filepath.Join(dataDir, "nuevoviruz.db")); err == nil {
...
func TestCmdCloudStatusDistinguishesAuthAndSyncReadiness(t *testing.T) {
	...
	t.Run("configured but missing token", func(t *testing.T) {
		t.Setenv("NV_CLOUD_TOKEN", "")
		t.Setenv("NV_CLOUD_INSECURE_NO_AUTH", "")
		withArgs(t, "engram", "cloud", "status")
	...
	t.Run("configured in insecure no-auth mode", func(t *testing.T) {
		t.Setenv("NV_CLOUD_TOKEN", "")
		t.Setenv("NV_CLOUD_INSECURE_NO_AUTH", "1")
		withArgs(t, "engram", "cloud", "status")
		stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if recovered != nil || stderr != "" {
			t.Fatalf("cloud status should succeed, panic=%v stderr=%q", recovered, stderr)
	...
	t.Run("configured with token", func(t *testing.T) {
		t.Setenv("NV_CLOUD_TOKEN", "token-abc")
		t.Setenv("NV_CLOUD_INSECURE_NO_AUTH", "")
		withArgs(t, "engram", "cloud", "status")
		stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
		if recovered != nil || stderr != "" {
...
func TestCmdCloudUpgradeDoctorRequiresProjectAndIsDeterministic(t *testing.T) {
	...
	t.Run("legacy payload gaps are surfaced by doctor and block bootstrap preflight", func(t *testing.T) {
		...
		db, err := sql.Open("sqlite", filepath.Join(cfg.DataDir, "nuevoviruz.db"))
		if err != nil {
...
func TestCloudDashboardDocsEnablementFlowIsExecutable(t *testing.T) {
	...
	t.Setenv("NV_JWT_SECRET", strings.Repeat("x", 32))
	t.Setenv("NV_CLOUD_TOKEN", "token-abc")
	t.Setenv("NV_CLOUD_ALLOWED_PROJECTS", "smoke-project")
	t.Setenv("NV_CLOUD_ADMIN", "token-abc")
	...
	if seen.AdminToken != "token-abc" {
		t.Fatalf("expected NV_CLOUD_ADMIN to flow into runtime config, got %q", seen.AdminToken)
	}
}
...
func TestCmdCloudStatusHonorsEnvServerOverride(t *testing.T) {
	...
	cfg := testConfig(t)
	t.Setenv("NV_CLOUD_SERVER", "https://env-cloud.example.test")
	t.Setenv("NV_CLOUD_TOKEN", "env-token")
...
func TestCmdCloudStatusRejectsInvalidEffectiveRuntimeServerURL(t *testing.T) {
	...
	t.Setenv("NV_CLOUD_SERVER", "https://env-cloud.example.test?debug=1")
	withArgs(t, "engram", "cloud", "status")
	stdout, stderr, recovered := captureOutputAndRecover(t, func() { cmdCloud(cfg) })
	if _, ok := recovered.(exitCode); !ok {
...
func TestCmdCloudServeStartsCloudRuntime(t *testing.T) {
	...
	t.Setenv("NV_CLOUD_TOKEN", "token-abc")
	t.Setenv("NV_CLOUD_ALLOWED_PROJECTS", "proj-a")
...
func TestCmdCloudServeRequiresAuthTokenByDefault(t *testing.T) {
	...
	if !strings.Contains(stderr, "NV_CLOUD_TOKEN") {
...
func TestCmdCloudServeRequiresProjectAllowlistByDefault(t *testing.T) {
	...
	cfg := testConfig(t)
	t.Setenv("NV_CLOUD_TOKEN", "token-abc")
	t.Setenv("NV_CLOUD_ALLOWED_PROJECTS", "")
	t.Setenv("NV_CLOUD_INSECURE_NO_AUTH", "")
	...
	if !strings.Contains(stderr, "NV_CLOUD_ALLOWED_PROJECTS") {
...
func TestCmdCloudServeInsecureModeRejectsDashboardAdminToken(t *testing.T) {
	...
	t.Setenv("NV_CLOUD_ALLOWED_PROJECTS", "proj-a")
	t.Setenv("NV_CLOUD_INSECURE_NO_AUTH", "1")
	t.Setenv("NV_CLOUD_ADMIN", "admin-token")
	...
	if !strings.Contains(stderr, "NV_CLOUD_ADMIN") || !strings.Contains(stderr, "NV_CLOUD_INSECURE_NO_AUTH") {
...
func TestCmdCloudServeAuthenticatedModeRequiresExplicitJWTSecret(t *testing.T) {
	...
	if !strings.Contains(stderr, "NV_JWT_SECRET") || !strings.Contains(stderr, "non-default") {
...
func TestCmdCloudServeAuthenticatedModeRejectsDefaultJWTSecret(t *testing.T) {
	...
	cfg := testConfig(t)
	t.Setenv("NV_JWT_SECRET", cloud.DefaultJWTSecret)
	t.Setenv("NV_CLOUD_TOKEN", "token-abc")
	t.Setenv("NV_CLOUD_ALLOWED_PROJECTS", "proj-a")
	t.Setenv("NV_CLOUD_INSECURE_NO_AUTH", "")
	...
	if !strings.Contains(stderr, "NV_JWT_SECRET") || !strings.Contains(stderr, "default") {
...
func TestCmdCloudServeRejectsInsecureModeWhenTokenIsSet(t *testing.T) {
	...
	if !strings.Contains(stderr, "NV_CLOUD_INSECURE_NO_AUTH") || !strings.Contains(stderr, "NV_CLOUD_TOKEN") {
...
func TestStoreSyncStatusProviderRejectsInvalidRuntimeServerURL(t *testing.T) {
	...
	t.Setenv("NV_CLOUD_SERVER", "https://cloud.example.test?debug=1")
...
func TestCmdExportDefaultAndCmdImportErrors(t *testing.T) {
	...
	if !strings.Contains(stdout, "Exported to nuevoviruz-export.json") {
		t.Fatalf("unexpected default export output: %q", stdout)
	}
	if _, err := os.Stat(filepath.Join(workDir, "nuevoviruz-export.json")); err != nil {
...
func TestMainDispatchServeMCPAndTUI(t *testing.T) {
	...
	t.Setenv("NV_DATA_DIR", t.TempDir())
...
func TestCmdSyncAdditionalBranches(t *testing.T) {
	...
	t.Run("status parse error", func(t *testing.T) {
		...
		if err := os.MkdirAll(filepath.Join(workDir, ".nuevoviruz"), 0755); err != nil {
			t.Fatalf("mkdir .nuevoviruz: %v", err)
		}
		if err := os.WriteFile(filepath.Join(workDir, ".nuevoviruz", "manifest.json"), []byte("{bad json"), 0644); err != nil {
...
func TestPreflightCloudSyncAllowsEmptyTokenWhenServerConfigured(t *testing.T) {
	...
	t.Setenv("NV_CLOUD_SERVER", "http://127.0.0.1:9090")
	t.Setenv("NV_CLOUD_TOKEN", "")
	t.Setenv("NV_CLOUD_INSECURE_NO_AUTH", "")
...
func TestCmdSyncCloudInvalidRuntimeServerURLMarkedConfigError(t *testing.T) {
	...
	t.Setenv("NV_CLOUD_SERVER", "://bad-url")
	t.Setenv("NV_CLOUD_TOKEN", "token-abc")
...
func TestCmdSyncCloudStatusPrintsProjectScopedChunkCounts(t *testing.T) {
	...
	t.Setenv("NV_CLOUD_SERVER", "https://cloud.example.test")
	t.Setenv("NV_CLOUD_TOKEN", "")
...
func TestCmdSyncImportEmptyAndMixedChunks(t *testing.T) {
	...
	t.Run("import with empty manifest", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(workDir, ".nuevoviruz", "manifest.json"), []byte(`{"version":1,"chunks":[]}`), 0644); err != nil {
			t.Fatalf("write manifest: %v", err)
		}
